/*******************************************************************************
* Copyright Regione Piemonte - 2023
* SPDX-License-Identifier: EUPL-1.2
******************************************************************************/


package it.csi.dma.apiopsan.external.scaricostudiservice.dma;

import java.util.ArrayList;
import java.util.List;
import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlSeeAlso;
import javax.xml.bind.annotation.XmlType;

import it.csi.dma.apiopsan.external.scaricostudiservice.dmass.AggiornaStatoRichiestaResponse;
import it.csi.dma.apiopsan.external.scaricostudiservice.dmass.CancellaPacchettoResponse;
import it.csi.dma.apiopsan.external.scaricostudiservice.dmass.Errore;
import it.csi.dma.apiopsan.external.scaricostudiservice.dmass.GetElencoPacchettiScadutiResponse;
import it.csi.dma.apiopsan.external.scaricostudiservice.dmass.ScaricoStudiResponse;
import it.csi.dma.apiopsan.external.scaricostudiservice.dmass.SetPacchettoCancellatoResponse;
import it.csi.dma.apiopsan.external.scaricostudiservice.dmass.VerificaStatoListaRichiesteResponse;
import it.csi.dma.apiopsan.external.scaricostudiservice.dmass.VerificaStatoRichiesta2Response;
import it.csi.dma.apiopsan.external.scaricostudiservice.dmass.VerificaStatoRichiestaResponse;


/**
 * <p>Classe Java per serviceResponse complex type.
 * 
 * <p>Il seguente frammento di schema specifica il contenuto previsto contenuto in questa classe.
 * 
 * <pre>
 * &lt;complexType name="serviceResponse"&gt;
 *   &lt;complexContent&gt;
 *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType"&gt;
 *       &lt;sequence&gt;
 *         &lt;element name="errori" type="{http://dmass.csi.it/}errore" maxOccurs="unbounded" minOccurs="0"/&gt;
 *         &lt;element name="esito" type="{http://www.w3.org/2001/XMLSchema}string" minOccurs="0" form="unqualified"/&gt;
 *       &lt;/sequence&gt;
 *     &lt;/restriction&gt;
 *   &lt;/complexContent&gt;
 * &lt;/complexType&gt;
 * </pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "serviceResponse", propOrder = {
    "errori",
    "esito"
})
@XmlSeeAlso({
    AggiornaStatoRichiestaResponse.class,
    CancellaPacchettoResponse.class,
    GetElencoPacchettiScadutiResponse.class,
    ScaricoStudiResponse.class,
    SetPacchettoCancellatoResponse.class,
    VerificaStatoListaRichiesteResponse.class,
    VerificaStatoRichiesta2Response.class,
    VerificaStatoRichiestaResponse.class
})
public class ServiceResponse {

    @XmlElement(nillable = true)
    protected List<Errore> errori;
    @XmlElement(namespace = "")
    protected String esito;

    /**
     * Gets the value of the errori property.
     * 
     * <p>
     * This accessor method returns a reference to the live list,
     * not a snapshot. Therefore any modification you make to the
     * returned list will be present inside the JAXB object.
     * This is why there is not a <CODE>set</CODE> method for the errori property.
     * 
     * <p>
     * For example, to add a new item, do as follows:
     * <pre>
     *    getErrori().add(newItem);
     * </pre>
     * 
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list
     * {@link Errore }
     * 
     * 
     */
    public List<Errore> getErrori() {
        if (errori == null) {
            errori = new ArrayList<Errore>();
        }
        return this.errori;
    }

    /**
     * Recupera il valore della proprietÃ  esito.
     * 
     * @return
     *     possible object is
     *     {@link String }
     *     
     */
    public String getEsito() {
        return esito;
    }

    /**
     * Imposta il valore della proprietÃ  esito.
     * 
     * @param value
     *     allowed object is
     *     {@link String }
     *     
     */
    public void setEsito(String value) {
        this.esito = value;
    }

}
