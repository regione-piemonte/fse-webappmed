...
@Service
public class DocumentiCorrelati extends LoggerUtil {
  ...
  //  private static final QName SERVICE_NAME_DOCUMENTI_INI = new QName("http://dmacc.csi.it/", "RicercaDocumentiINIService");
	...
	public Response execute(String shibIdentitaCodiceFiscale, String xRequestId, String xForwardedFor,
	    ...
	    long id_xml = codLMessaggiService.traceMessaggiInsert(shibIdentitaCodiceFiscale, xRequestId, xForwardedFor, 
	    		...
	    		httpHeaders, httpRequest, uuidAsString, Constants.GET_DOCUMENTICORRELLATI,citId);
	    //inserisco tabella dei log
	    long idlog = codRLogErroreService.traceLogInsert(Constants.GET_DOCUMENTICORRELLATI, uuidAsString, Constants.CITTADINI_API_SERVICE,null);
	    List<it.csi.dma.apiopsan.documentiservice.dmacc.Documento> documentiResponse = null;

	    List<SintesiDocumentoEsteso> doc= new ArrayList<SintesiDocumentoEsteso>();
	    List<SintesiDocumentoEsteso> temp= new ArrayList<SintesiDocumentoEsteso>();
		try {
		...
		// validate
			List<ErroreDettaglioExt> listError = validateMedicGeneric.validateDocumentiCorrellati(shibIdentitaCodiceFiscale, xRequestId, 
					xForwardedFor, xCodiceServizio, xCodiceVerticale, ruolo,ruoloFse,regime, collocazione,
					 citId, idDocumentoIlec, codiceComponenteLocale,tipoCorrelazioneDocumento, securityContext, httpHeaders, httpRequest);
...
//					o	<consensoConsultazione>=True allora il sistema procede con lâindividuazione degli episodi da restituire
					...
					doc= dmaccIdxDao.selectDocumentiCorrealati(idDocumentoIlec, codiceComponenteLocale, tipoCorrelazioneDocumento);
					if (doc.isEmpty()) {
						//dai errore e accoda ad errori
					...
					return Response.status(200).entity(doc).build();
						...
						//procedo con controllo  documenti oscurati
						for (int i = 0; i < size; i++) {
							SintesiDocumentoEsteso d= doc.get(i);
							if(! statoDocumento.isOscurato(d.getIdDocumentoIlec(), d.getCodiceCl(), d.getCitId()) ) {
								temp.add(d);
							}
						}
					 ...
					 List<SintesiDocumentoEsteso> listaDocFiltrata= new ArrayList<SintesiDocumentoEsteso>();
					 if(!temp.isEmpty())
						 temp.removeIf(d-> (d.getIdDocumentoIlec().equals(idDocumentoIlec))
					 );
					for (SintesiDocumentoEsteso d : temp) {
						SintesiDocumentoEsteso selectDocumento = dmaccIdxDao.selectDocumento(d.getIdDocumentoIlec(), d.getCodiceCl());
						if (selectDocumento != null)
					 listaDocFiltrata.add(selectDocumento);
					}

				codRLogErroreService.traceLogInsert(Constants.GET_DOCUMENTICORRELLATI, uuidAsString, Constants.CITTADINI_API_SERVICE,Constants.ESITO_SUCCESSO);
				codLMessaggiService.traceMessaggiUpdate(id_xml, uuidAsString, listaDocFiltrata.toString(),Constants.ESITO_SUCCESSO);
				codRLogErroreService.traceLogAuditInsert(Constants.GET_DOCUMENTICORRELLATI, uuidAsString, listaDocFiltrata.toString(), 
						Constants.CODICE_SUCCESSO_DOCUMENTICORRELATI, shibIdentitaCodiceFiscale, 
						collocazione, xForwardedFor, xCodiceVerticale, xCodiceServizio, ruoloFse, regime, citId,null,null,null);
				 return Response.status(200).entity(listaDocFiltrata).build();
		...
		codRLogErroreService.traceLogInsert(Constants.GET_DOCUMENTICORRELLATI, uuidAsString, Constants.CITTADINI_API_SERVICE,Constants.ESITO_FALLIMENTO);
		return esito;
