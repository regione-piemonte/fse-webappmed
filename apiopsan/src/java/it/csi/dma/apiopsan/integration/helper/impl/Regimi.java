...
@Service
public class Regimi extends LoggerUtil {
	...
	public Response execute(String shibIdentitaCodiceFiscale, String xRequestId, String xForwardedFor,
	    ...
	    long id_xml = codLMessaggiService.traceMessaggiInsert(shibIdentitaCodiceFiscale, xRequestId, xForwardedFor, 
	    		...
	    		httpHeaders, httpRequest, uuidAsString, Constants.GET_REGIMI,null);
	    //inserisco tabella dei log
	    long idlog = codRLogErroreService.traceLogInsert(Constants.GET_REGIMI, uuidAsString, Constants.OPERATORE_SANITARIO_SERVICE,null);
		try {
		...
		// validate
			List<ErroreDettaglioExt> listError = validateMedicGeneric.validate(shibIdentitaCodiceFiscale, xRequestId, 
					xForwardedFor, xCodiceServizio, xCodiceVerticale, ruolo,ruoloFse,collocazione, securityContext, httpHeaders, httpRequest);
			if (!listError.isEmpty()) {
				throw new ResponseErrorException(
						ErrorBuilder.generateErrorBuilderError(StatusEnum.BAD_REQUEST, listError),"errore in validate");
			}
			
			List<Codice> listaRegimi = new ArrayList<Codice>(); 
			listaRegimi = getRegimiDao.selectRegimiPerRuolo(xCodiceServizio, ruolo);
			
			//inserimento nel log esito positivo
			codRLogErroreService.traceLogInsert(Constants.GET_REGIMI, uuidAsString, Constants.OPERATORE_SANITARIO_SERVICE,Constants.ESITO_SUCCESSO);
			codLMessaggiService.traceMessaggiUpdate(id_xml, uuidAsString, listaRegimi.toString(),Constants.ESITO_SUCCESSO);
			//popolare la tabella di audit solo se esito positivo
			codRLogErroreService.traceLogAuditInsert(Constants.GET_REGIMI, uuidAsString, listaRegimi.toString(), 
					Constants.CODICE_SUCCESSO_GETREGIMI, shibIdentitaCodiceFiscale, 
					collocazione, xForwardedFor, xCodiceVerticale, xCodiceServizio, ruoloFse, null, null,null,null,null);
			
			 return Response.status(200).entity(listaRegimi).build();
		...
		codRLogErroreService.traceLogInsert(Constants.GET_REGIMI, uuidAsString, Constants.OPERATORE_SANITARIO_SERVICE,Constants.ESITO_FALLIMENTO);
		return esito;
